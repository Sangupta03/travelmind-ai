...
"""
        ...
        # RETURN TO HOTEL
        # -----------------------------
        return_decision = self.transport_engine.decide(ordered_places[-1], start_point)
        return_mode = "walking" if return_decision["mode"] == "WALK" else "driving"
        return_link = self.route_links.generate(ordered_places[-1], start_point, return_mode)

        final_plan += f"""
End of Day Return:
From: {ordered_places[-1]}
To: {hotel_name}
Mode: {return_decision['mode']}
Time: {return_decision['time']}
Google Maps Route: {return_link}
"""
